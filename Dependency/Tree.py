...
#		sys.stderr.write('Reading trees from %s...' % path)
		for i,line in enumerate(fIn.readlines()):
			...
			if (i+1)%1000 == 0:
				sys.stderr.write('reading %d trees...\r' % (i+1))
...
#-----------------------------------------------------------------------------------------------
	def DisTreeStructure(self,  fOut = sys.stderr,  outEncoding = 'gbk',  nSpace = 0):
		...
		if (self.IsPreterminal() == True):
			fOut.write((str  + u'[' + self.m_val + '\t' + self.m_children[0].m_val + ']').encode(outEncoding) + '\n')
			return
...
#-----------------------------------------------------------------------------------------------
	def GetTagWordList(self):
		...
		wordTagList = [(node.GetChild(0).GetVal(), node.GetVal()) for node in preTerminals]
		return wordTagList
